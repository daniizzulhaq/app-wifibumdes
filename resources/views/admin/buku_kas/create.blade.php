...
@section('page-title', 'Tambah Buku Kas')
...
<div class="container-custom">
    ...
    <div class="header-section">
        <div class="header-title">
            ...
            <p>Isi data pemasukan atau pengeluaran baru</p>
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3><i class="fas fa-plus-circle"></i> &nbsp;Form Transaksi Baru</h3>
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.buku_kas.store') }}">
                @csrf
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="nominal" class="form-control @error('nominal') border-red-500 @enderror"
                               value="{{ old('nominal') }}" min="0" placeholder="Contoh: 500000">
                        @error('nominal')
                            <div class="form-error"><i class="fas fa-exclamation-circle"></i> {{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group form-group-full">
                        ...
                        <div class="radio-group">
                            ...
                            <label class="radio-card {{ old('jenis') == 'pemasukan' ? 'selected-masuk' : '' }}" id="card-masuk">
                            ...
                            <label class="radio-card {{ old('jenis') == 'pengeluaran' ? 'selected-keluar' : '' }}" id="card-keluar">
                    ...
                    <div class="form-group">
                        ...
                        <select name="kategori" class="form-control @error('kategori') border-red-500 @enderror">
                            ...
                            <option value="stock_barang"     {{ old('kategori') == 'stock_barang'     ? 'selected' : '' }}>Stock Barang</option>
                            <option value="tagihan_banwith"  {{ old('kategori') == 'tagihan_banwith'  ? 'selected' : '' }}>Tagihan Bandwidth</option>
                            <option value="honor_karyawan"   {{ old('kategori') == 'honor_karyawan'   ? 'selected' : '' }}>Honor Karyawan</option>
                            <option value="sosial"           {{ old('kategori') == 'sosial'           ? 'selected' : '' }}>Sosial</option>
                            <option value="donatur"          {{ old('kategori') == 'donatur'          ? 'selected' : '' }}>Donatur</option>
                            <option value="listrik"          {{ old('kategori') == 'listrik'          ? 'selected' : '' }}>Listrik</option>
                            <option value="bpjs"             {{ old('kategori') == 'bpjs'             ? 'selected' : '' }}>BPJS</option>
                            ...
                            <option value="thr"              {{ old('kategori') == 'thr'              ? 'selected' : '' }}>THR</option>
                            <option value="lain_lain"        {{ old('kategori') == 'lain_lain'        ? 'selected' : '' }}>Lain - Lain</option>
                        </select>
                        @error('kategori')
                            <div class="form-error"><i class="fas fa-exclamation-circle"></i> {{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="keterangan" class="form-control"
                                  placeholder="Keterangan tambahan (optional)">{{ old('keterangan') }}</textarea>
...
<script>
    ...
    // Jalankan saat load kalau ada old value
