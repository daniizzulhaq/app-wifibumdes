        ...
        <form method="GET" action="{{ route('pelanggan.tagihan.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <option value="nunggak" {{ request('status') == 'nunggak' ? 'selected' : '' }}>Belum Bayar</option>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-md p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-red-100 text-sm mb-1">Total Belum Bayar</p>
                    <h3 class="text-3xl font-bold">{{ $stats['belum_bayar'] ?? 0 }}</h3>
                </div>
            ...
            <p class="text-red-100 text-sm mt-3">
                Rp {{ number_format($stats['total_belum_bayar'] ?? 0, 0, ',', '.') }}
            </p>
        </div>
